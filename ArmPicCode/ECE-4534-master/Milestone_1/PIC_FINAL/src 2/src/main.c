...
void main(void) {
   ...
   // timer1_thread_struct t1thread_data; // info for timer1_lthread
    ...
    //OSCTUNEbits.PLLEN = 0; clock set to 12MHz
    //OSCTUNEbits.PLLEN = 1; clock set to 48MHz
    OSCTUNEbits.PLLEN = 1;
    ...
    OpenADC(ADC_FOSC_16  & ADC_RIGHT_JUST & ADC_12_TAD, ADC_CH0 & ADC_INT_ON & ADC_VREFPLUS_VDD & ADC_VREFMINUS_VSS, ADC_0ANA);
    ADC_INT_ENABLE();
    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                case MSGT_I2C_DATA:
                {
                  //When the master on I2C writes something
                  //Transmit that on the uart

                  unsigned short msgId = msgbuffer[0] >> 4;
                  if (msgId != 0x1)
                    uart_write(length-1,msgbuffer);
                  //flipDBG(0);
                  break;
                };
                case MSGT_I2C_DBG:
                ...
                case MSGT_I2C_RQST:
                {
                   ...
                   // length = 6;
                    //start_i2c_slave_reply(I2C_Buffer_Size, msgbuffer);
                    break;
                };
                default:
                {
        ...
        if (length < 0) {
            ...
            switch (msgtype) {
                ...
                case MSGT_I2C_DATA_SAVE:
                {
                    //Handle_i2c_data_save(1,msgbuffer);
                    break;
                }
                case MSGT_UART_DATA:
                {
                    ...
                    //unsigned char i = 0;
                    //for(i = 0; i < length; i++){
                    //    Handle_i2c_data_save(1, msgbuffer + i);
                    //}

                    WriteI2CBuffer(length, msgbuffer);
