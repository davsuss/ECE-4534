...
signed char send_msg_pointer(msg_queue *qptr, unsigned char length, unsigned char msgtype, void *data){
     unsigned char slot;
...
signed char recv_msg_pointer(msg_queue *qptr, unsigned char maxlength, unsigned char *msgtype, void *data){
    unsigned char slot;
    //unsigned char	i;
...
signed char ToMainLow_sendmsg_pointer(unsigned char length, unsigned char msgtype, void *data){
#ifdef DEBUG
    if (!in_low_int()) {
        return (MSG_NOT_IN_LOW);
    }
    ...
    return (send_msg_pointer(&ToMainLow_MQ, length, msgtype, data));
}

signed char ToMainLow_recvmsg_pointer(unsigned char maxlength, unsigned char *msgtype, void *data){
#ifdef DEBUG
    if (!in_main()) {
        return (MSG_NOT_IN_MAIN);
    }
#endif
    return (recv_msg_pointer(&ToMainLow_MQ, maxlength, msgtype, data));
}
