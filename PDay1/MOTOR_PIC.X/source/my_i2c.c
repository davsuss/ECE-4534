...
void start_i2c_slave_reply(unsigned char length, unsigned char *msg) {
    ...
    int feetL = DeltaOverflowL / 0x08;
    int inchesL = DeltaOverflowL % 0x08;
    int feetR = DeltaOverflowR / 0x08;
    int inchesR = DeltaOverflowR % 0x08;

    unsigned int distance = motorDistance + ReadTimer1();
    ic_ptr->outbuffer[0] = ActiveL | ActiveR;
    ic_ptr->outbuffer[1] = feetL;
    ic_ptr->outbuffer[2] = inchesL;
    ic_ptr->outbuffer[3] = feetR;
    ic_ptr->outbuffer[4] = inchesR;
...
void i2c_slave_int_handler() {
    ...
    if (msg_ready) {
        ...
        ToMainHigh_sendmsg(sizeof(unsigned char) * (ic_ptr->buflen + 1), MSGT_MOTOR_COMMAND, (void *) ic_ptr->buffer);
        ic_ptr->buflen = 0;
